...
class DscAppPanel extends JPanel implements InstanceContext,
		InternalBusListener, Constants {
	...
	public MultiBeeper getBeeper() {
		...
		if (_oBeeper == null)
			throw new RuntimeException(
					"DscAppPanel.getBeeper = Beeper is null.");
	...
	public void updateSignal(Dscmessage oDscMessage) {
		...
		AppLogger.debug("applet.DscAppPanel.updateSignal="
				+ oDscMessage.toString());
		...
		if ((myMMSI.equals(msgFromMMSI) == false)) {
			if ((CALL_TYPE_DISTRESS.equals(msgType) || CALL_TYPE_DISTRESS_ACK
					.equals(msgType))
					|| (CALL_TYPE_ALL_SHIPS.equals(msgType) || CALL_TYPE_ALL_SHIPS_ACK
							.equals(msgType))
					|| ((hasToMMSI && myMMSI.equals(msgToMMSI) == true))
					&& (CALL_TYPE_INDIVIDUAL.equals(msgType) || CALL_TYPE_INDIVIDUAL_ACK
							.equals(msgType))
					|| ((hasToMMSI && myGroupMMSI.equals(msgToMMSI) == true) && (CALL_TYPE_GROUP
							.equals(msgType) || CALL_TYPE_GROUP.equals(msgType)))) {

				//ContentManager.storeLastDistressCalls(oDscMessage);
				getContentManager().storeCallMessage(
						(Dscmessage) oDscMessage.clone());
